		...
		for (GrafanaQueryTargets target : input.getTargets()) {
			...
			for (int i = 0; i < output.size(); i++) {
				...
				if (i == 0) {
					...
					tmp_list.add(new String[] { values, to_grafana_timestamp(s_array[0]),
							df.format(Double.parseDouble(s_array[1])).toString().replace(',', '.') });
					data_values.add(tmp_list);
				} else {
					String[] s_array_davor = output.get((i - 1));

					if (s_array[2].equals(s_array_davor[2]) && s_array[3].equals(s_array_davor[3])) {
						...
						tmp_list.add(new String[] { values, to_grafana_timestamp(s_array[0]),
								df.format(Double.parseDouble(s_array[1])).toString().replace(',', '.') });
						// Add last request timestamp to list -> data will be
						// generate till timerange end
						tmp_list.add(new String[] { values, unixTo + "000",
								df.format(Double.parseDouble(s_array[1])).toString().replace(',', '.') });
						data_values.add(tmp_list);
						list_i++;
					}
